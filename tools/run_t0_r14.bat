@echo off
rem T0 round 14 (final static): d30 with WEIGHTS forced on-chip (no flash pool).
setlocal enabledelayedexpansion
set STEDGEAI=C:\ST\STEdgeAI\4.0\Utilities\windows\stedgeai.exe
cd /d "%~dp0\.."
set RUN=models\t0\out\r14_%RANDOM%
mkdir %RUN%
echo Output: %RUN%
call :one d30_bh_c192_v3_l12_qdq n6-nucleo-full-onchip "--input-data-type int8 --output-data-type int8 --enable-epoch-controller" d30_allonchip_ec
