...
*This is a translation of the Russian original published on Habr
(<https://habr.com/ru/sandbox/299290/>), done with the help of Claude
(Anthropic). Numbers, identifiers, file names and quoted listening notes
are preserved exactly as in the original.*
...
I wrote the first version with an `i`, as in the kettledrum, then renamed it: on a cicada it is `tymbal`, with a `y`.
...
(STM32N657: Cortex-M55 at 800 MHz + Neural-ART neural accelerator). Control is an Arturia KeyStep MIDI keyboard over DIN, output is an I2S DAC. The aesthetic is
synthetic glitch melodism in the spirit of Alva Noto × Ryuichi Sakamoto, deliberately
...
**A pipeline of depth one.** While the NPU computes block i−1, the processor
...
optimal linear FIR of 520 coefficients (least squares on the same corpus) for all six teacher candidates: "network − FIR" from −0.94 to −3.73 dB. 281
...
What accumulated over the bring-up (the full journal is in the repository on GitHub — docs/chip_findings.md,
docs/m2_notes.md):
...
The AXI cache on this profile is a penalty: +335K cycles per hop. The buffers
...
**2 473 000**. The stage itself after MVE is 245K; the first, scalar version cost 652K, and it is covered in §8. The budget is 3 200 000; headroom is
...
- "The FIR is ~3% of the budget even in scalar form" — it turned out to be 20%, 652K cycles, 6.5 per MAC. A dot product "backwards along the signal",
...
> "The network gives roundness and velvet, plushness. Plus the
> attack came out with an overtone of an organ or a shakuhachi, with that
> overblowing you get on wind instruments. The FIR is less plush, but it's different from DRY too."
...
> "The teacher has the overblow too. The character is reproduced.
> The midrange is pretty similar. The teacher has the grit too. The sound came out a bit brighter than the teacher."
...
does not reach as far as structure. By the criterion written down before the listening test, this means an honest ceiling: overfitting is not starting.
...
> different from the one before, and the chords play and shimmer in an interesting way."
...
Both thoughts are about behavior in time. Nothing about timbre, and that is
exactly what the skeleton was missing when it sounded like 8-bit audio out of
Mario.

### Skeleton noise (7 Aug)

The first thing I thought about the sound after a day of playing: quite a lot of
noise, grit running parallel to every sounding voice.

"Parallel" is diagnostic here. The noise is tied to the voice's
...
level of 0.15 × timbreB × envelope is added to each subband. The coefficient is the same for all four PQMF subbands, whereas the harmonic part falls off
...
I do not have an oscilloscope. I found that it is not needed either: in this chain
...
The instrument assembled. An Arturia KeyStep as the keyboard
...
Five wires from the pin header to the DAC — BCK to D12, LRCK to
...
and without this jumper the DAC's internal PLL does not start — the module stays silent while looking perfectly healthy.
...
The repository: <https://github.com/mikekss/tymbal>

`dsp/` — Python references with self-checks; `train/` —
